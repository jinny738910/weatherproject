...
android {
    ...
    defaultConfig {
        ...
        buildConfigField ("String", "WEATHER_API_KEY",
                "\"${localProperties.getProperty("WEATHER_API_KEY", "default_or_placeholder_key")}\"")
        buildConfigField ("String", "GPT_API_KEY",
                "\"${localProperties.getProperty("GPT_API_KEY", "default_or_placeholder_key")}\"")
    }
...
//    kapt "androidx.room:room-compiler:2.2.6"
    ...
    // 필수 Compose 라이브러리 (버전은 BOM이 관리)
    implementation("androidx.compose.ui:ui")
    implementation("androidx.compose.material3:material3") // Material Design 3
    implementation("androidx.compose.ui:ui-tooling-preview")
    implementation("androidx.activity:activity-compose")    // Activity와 Compose 통합

    // 디버그 빌드에서만 필요한 의존성 (미리보기, Live Edit 등)
    ...
    // 기타 기존 의존성 (Core KTX, Lifecycle 등)
    ...
    // 1. Retrofit (핵심 라이브러리)
    implementation("com.squareup.retrofit2:retrofit:2.9.0")
    // 2. Gson Converter (JSON 데이터 파싱용)
    implementation("com.squareup.retrofit2:converter-gson:2.9.0")
    implementation("com.squareup.retrofit2:converter-scalars:2.9.0") // 필수
    // 3. OkHttp Logging Interceptor (네트워크 통신 로그 확인용 - 권장)
