...
//    ActivityResultLauncher를 생성하고 결과를 처리할 콜백을 정의합니다.
    ...
    private fun initViews(binding: ActivityListBinding) = with(binding) {
        ...
        addToDoButton.setOnClickListener {
            val intent = Intent(this@AlarmListActivity, DetailActivity::class.java)
            detailLauncher.launch(
                DetailActivity.getIntent(this@AlarmListActivity, DetailMode.WRITE)
            )
        }

        weatherDetailsButton.setOnClickListener {
            val intent = Intent(this@AlarmListActivity, WeatherActivity::class.java)
